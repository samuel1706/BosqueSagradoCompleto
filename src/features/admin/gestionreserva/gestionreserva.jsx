...
import { FaEye, FaEdit, FaTrash, FaTimes, FaExclamationTriangle, FaPlus, FaMoneyBillAlt, FaFilePdf, FaCalendarAlt, FaUser, FaMapMarkerAlt, FaHome, FaBox, FaCreditCard, FaSearch, FaSync, FaCheck, FaReceipt } from "react-icons/fa";
...
// ===============================================
// COMPONENTE PRINCIPAL GestionReserva CORREGIDO
// ===============================================
const GestionReserva = () => {
  ...
  const obtenerAbonosReserva = async (idReserva) => {
    try {
      const res = await axios.get(API_ABONOS, { timeout: 10000 });
      const datos = Array.isArray(res.data) ? res.data
        : (res.data && Array.isArray(res.data.$values) ? res.data.$values : []);
      return datos.filter(a => a.idReserva != null && a.idReserva.toString() === idReserva.toString());
    } catch (error) {
      console.warn("⚠ Error al obtener abonos de reserva:", error);
      return [];
    }
  };

  const obtenerVentaReserva = async (idReserva) => {
    try {
      const res = await axios.get(API_VENTAS, { timeout: 10000 });
      const datos = Array.isArray(res.data) ? res.data
        : (res.data && Array.isArray(res.data.$values) ? res.data.$values : []);
      return datos.find(v => v.idReserva != null && v.idReserva.toString() === idReserva.toString());
    } catch (error) {
      console.warn("⚠ Error al obtener venta de reserva:", error);
      return null;
    }
  };
  ...
  const crearAbonoInicial = async (idReserva, montoAbono) => {
    try {
      ...
      const payload = {
        ...
        idMetodoPago: Number(newReserva.idMetodoPago),
        verificacion: "Pendiente"
      };

      await axios.post(API_ABONOS, payload, {
        headers: { 'Content-Type': 'application/json' }
      });
    } catch (error) {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (isEditing) {
          ...
          // Si no podemos obtener el ID de la respuesta, buscamos la reserva recién creada
          if (!idNumeric || isNaN(idNumeric)) {
            ...
            const found = lista.find(r => {
              ...
              return fechaRegApi === normalizeDate(newReserva.fechaRegistro)
                ...
                && Math.abs(Number(r.montoTotal ?? 0) - Number(reservaData.montoTotal)) < 1;
            });
          ...
          if (idNumeric && !isNaN(idNumeric)) {
            ...
            // Crear abono inicial
            await crearAbonoInicial(idNumeric, newReserva.abono);
            
            // Crear venta
            await crearVenta(idNumeric);
          }
        }
    ...
    if (reservaToDelete) {
      ...
      try {
        ...
        try {
          ...
          const abonosReserva = await obtenerAbonosReserva(idReserva);
          for (const abono of abonosReserva) {
            await axios.delete(`${API_ABONOS}/${abono.idAbono}`);
          }

          // Eliminar venta asociada
          const ventaReserva = await obtenerVentaReserva(idReserva);
          if (ventaReserva) {
            await axios.delete(`${API_VENTAS}/${ventaReserva.idVenta}`);
          }
        } catch (error) {
          console.warn("⚠ No se pudieron eliminar datos asociados:", error);
        }
  ...
  const validateForm = () => {
    const requiredFields = [
      ...
      'montoTotal', 'abono', 'restante',
      'idUsuario', 'idEstado', 'idSede', 'idCabana', 'idMetodoPago'
    ];

    for (const field of requiredFields) {
      if (!newReserva[field]) {
        displayAlert(`❌ El campo ${field.replace('id', '').replace('fechaEntrada', 'Fecha Entrada').replace('montoTotal', 'Monto Total')} es obligatorio.`);
        return false;
      }
    }
    ...
    if (parseFloat(newReserva.montoTotal) <= 0) {
  ...
  const handleView = async (reserva) => {
    try {
      const [serviciosNormalizados, abonosReserva, ventaReserva] = await Promise.all([
        obtenerServiciosReserva(reserva.idReserva).then(servicios => servicios.map(s => s.idServicio)),
        obtenerAbonosReserva(reserva.idReserva),
        obtenerVentaReserva(reserva.idReserva)
      ]);
  ...
  const handleDownloadPDF = async (reserva) => {
    ...
    try {
      const [serviciosReserva, abonosReserva] = await Promise.all([
        obtenerServiciosReserva(reserva.idReserva),
        obtenerAbonosReserva(reserva.idReserva)
      ]);
      ...
      abonosDetalle = abonosReserva.map(a => ({
        fecha: formatDate(a.fechaAbono),
        monto: a.montoAbono,
        metodo: getMetodoPagoNombre(a.idMetodoPago),
    ...
    doc.setTextColor(0, 0, 0);
    doc.text(formatCurrency(reserva.montoTotal), 120, y + 8);
    ...
    if (abonosDetalle.length > 0) {
      ...
      const abonosTable = abonosDetalle.map(abono => [
        abono.fecha,
      ...
      const totalAbonado = abonosDetalle.reduce((total, abono) => total + (abono.monto || 0), 0);
      abonosTable.push(['TOTAL ABONADO', formatCurrency(totalAbonado), '', '']);
  ...
  // ===============================================
  // COMPONENTES DE MODALES - MEJORADOS
  // ===============================================
  const DetailsModal = () => {
        ...
        <div style={detailsModalStyle}>
          ...
          <div>
            ...
            <div style={detailItemStyle}>
              ...
              <div style={detailValueStyle}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div><strong>Monto Total:</strong> {formatCurrency(currentReserva.montoTotal)}</div>
        ...
        <div style={{ ...modalContentStyle, maxWidth: 480, textAlign: 'center' }}>
          ...
          <div style={{ 
            ...
            <div style={{ fontSize: '13px', color: '#666' }}>
              ...
              <div>• Monto: {formatCurrency(reservaToDelete.montoTotal)}</div>
                    ...
                    <tr key={reserva.idReserva} style={{ 
                      ...
                      <td style={{ padding: "20px 16px", textAlign: "center" }}>
                        <div style={{ 
                          ...
                          {formatCurrency(reserva.montoTotal)}
                        </div>
